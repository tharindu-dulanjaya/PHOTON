...
$db = dbConn();
extract($_GET); // to get categoryId

// products are grouped by same item & same price. group has a common stock id (first id of the group)
// if we use any aggregate functions like SUM, we must write the GROUP BY

if ($categoryId == '0') { // load all products
    $where = "WHERE items.status = '1' AND item_category.status='1'";
} else {
    $where = "WHERE items.item_category='$categoryId' AND items.status = '1' AND item_category.status='1'";
}
