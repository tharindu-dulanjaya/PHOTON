...
$link = "New Customer Visit";
$breadcrumb_item = "Customer Visit";
$breadcrumb_item_active = "New";

// check the CRUD privileges for the logged in user
$privilege = checkprivilege('14'); 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($message)) {
        ...
        $sql = "INSERT INTO users(UserType, TitleId, FirstName, LastName, Email, Nic, AddressLine1, AddressLine2, City, MobileNo, GenderId, DistrictId, Token) "
                . "VALUES ('customer','$title','$first_name','$last_name','$email','$nic','$address_line1','$address_line2','$city','$mobile_no','$gender','$district','$token')";
        $db->query($sql);
        ...
        $msg .= "<h5>You are halfway there.. Please complete your registration by creating a username and password for your account.</h5>";
        $msg .= "<p>Use the below link to create username and password.</p>";
        $msg .= "<a href='http://localhost/photon/web/complete_registration.php?token=$token'>Complete registration now</a>";
        sendEmail($email, $first_name, "Complete Registration", $msg);

        echo "<script>
                    Swal.fire({
                        icon: 'success',
                        title: 'Registration Successful',
                        text: 'An email has been sent to the customer email address.',
                        showCloseButton: false,
                        showConfirmButton: false,
                        timer: 4000
                        }).then(function() {
                            window.location.href = 'http://localhost/photon/system/customers/manage.php';
                        });
                    </script>";
    }
}
    ...
    <div class="col-12">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">Add New Visit</h3>
            </div>
            <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" novalidate>
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-5 ">
                            ...
                            <input type="text" class="form-control border border-1 border-dark-subtle" name="telno" id="telno" placeholder="+94112587239" value="<?= @$telno ?>" required>
                ...
                <div class="card-footer text-right">
                    <button type="submit" class="btn btn-primary" <?= $privilege['Add'] == '0' ? 'disabled' : '' ?>>Add Customer</button>
                </div>
            </form>            
        </div>
    </div>
