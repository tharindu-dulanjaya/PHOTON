...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($message)) {
        ...
        /*
          ...
          // select the records that belong to the product
          $sql = "SELECT * FROM item_images WHERE ItemId = '$product_id'";
          $result = $db->query($sql);
          if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) { // loop the found no of records
          $img_id = $row['ImageId'];
          if ($value['upload']) {
          $extra_img = $value['file'];
          echo $sql = "UPDATE item_images SET ImagePath='$extra_img' WHERE ImageId='$img_id'";
          $db->query($sql);
          } else {
          ...
          } else { // no existing records.. so we insert new record
                    ...
                    if ($value['upload']) {
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                $img_id = $row['ImageId'];
                                $sql = "UPDATE item_images SET ImagePath='$extra_img' WHERE ImageId='$img_id'";
                                if ($db->query($sql)) {
                                    echo "Image updated successfully for ImageId: $img_id<br>";
                                } else {
                                    echo "Error updating image: " . $db->error . "<br>";
                                }
                            }
                        } else {
                            // Insert new records
